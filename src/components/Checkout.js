...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    const cartBalace = getTotalCartValue(items)
    if(cartBalace >= balance) {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items, addresses)) {
      let cartBalance = getTotalCartValue(items)
    let wallet = localStorage.getItem("balance")
    localStorage.setItem("balance", (wallet - cartBalance))
              ...
              {addresses.all.length === 0 && <Typography my="1rem">
               ...
               >
                {addresses.all.map((address) => (
                  ...
                  >
                    <p>{address.address}</p>
                    <Button onClick={async () => {await deleteAddress(token, address._id)}}>
                    <Delete/>
                    Delete
                    </Button>
